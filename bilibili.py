...
def download_video(video_url, dir_, video_name, index, count):
    ...
    video_path = os.path.join(dir_, video_name, video_name+'_'+str(index) + '.mp4')
    for i, mode in enumerate(video_mode):
        ...
        if response.status_code == 200:
            ...
            print("进程ID%d: %s 第%d/%d个片段, [文件大小]:%0.2f MB" % (
            os.getpid(), video_name, index, count, content_size / 1024 / 1024))
            with open(video_path, 'wb') as file:
                ...
                for data in response.iter_content(chunk_size=chunk_size):
                    ...
                    file.flush()
                    if j > 0 and j % 1000 == 0:
                        print("进程ID%d: %s 第%d/%d个片段, [下载进度]:%.2f%%" % (
                        os.getpid(), video_name, index, count, float(size / content_size * 100)))
                    j += 1
            return
...
def one_process(dir_, urls_and_titles):
    """
        ...
        此函数为一个进程所执行的部分下载任务
    """
    for urls, title,title1 in urls_and_titles:
        download_videos(dir_, urls, '%s' % title+"_"+title1)
...
if __name__ == '__main__':
    ...
    n = 10  # 大列表中几个数据组成一个小列表
    all_list = [data_list[i:i + n] for i in range(0, len(data_list), n)]
    ...
    for i in range(1):
        ...
        data_id = re.search("\d+", all_list[aa][i]['av号'])
        aid = '3909884'
        ...
        urls_and_titles = []
        # 重点
        for title1, page in title_pages.items():
            if page < start_page:
                continue
            video_url = url + '/?p=%d' % page
            urls = get_download_urls(video_url)
            urls_and_titles.append([urls, title,title1])
